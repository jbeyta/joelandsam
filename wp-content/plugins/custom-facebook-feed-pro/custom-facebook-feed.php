...
define( 'WPW_SL_ITEM_NAME', 'Custom Facebook Feed WordPress Plugin Developer' ); //Update #
...
function display_cff($atts, $content = null) {
    ...
    array(
        ...
        //Photos only
        'photosource' => $options[ 'cff_photos_source' ],
        'photocols' => $options[ 'cff_photos_cols' ],
        //Filters
        ...
        'expandcomments' => $options[ 'cff_expand_comments' ],
        'commentsnum' => $options[ 'cff_comments_num' ],
        'hidecommentimages' => $options[ 'cff_hide_comment_avatars' ],
    ...
    if ($access_token == '' || !$cff_show_access_token) $access_token = '611606915581035|RdRHbHtrHseQw4C7SDUBFWIrJLA';
    ...
    //Lightbox extension
    if ( $atts['lightbox'] == 'true' || $atts['lightbox'] == 'on' ) $cff_content .= 'cff-lightbox';
    ...
    foreach ( $page_ids as $page_id ) {
        ...
        if ($cff_events_only && $cff_events_source == 'eventspage'){
            ...
            //Past events
            if( $atts['pastevents'] !== 'false' ) $cff_past_events = true;
        ...
        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
                ...
                if ( $cff_show_post ) {
                    ...
                    if($cff_albums_only && $cff_albums_source == 'photospage'){
                        ...
                        if($cff_is_group){
                            ...
                            if($cff_show_album_title) $cff_post_item .= '<h4><a href="' . $cff_album_link . '" '.$target.'>' . $cff_album_name . '</a></h4>';
                            if( $cff_show_album_number && isset($news->count) ) $cff_post_item .= '<p>' . $cff_album_count . ' photos</p>';
            ...
            } else if( $cff_album_active && !empty($cff_album_id) ) {
                //ALBUM EMBED
...
function cff_add_my_stylesheet() {
    ...
    wp_register_style( 'cff', plugins_url('css/cff-style.css?11', __FILE__) );
...
function cff_scripts_method() {
    ...
    wp_register_script( 'cffscripts', plugins_url( '/js/cff-scripts.js?11' , __FILE__ ), array('jquery'), '1.8', true );
