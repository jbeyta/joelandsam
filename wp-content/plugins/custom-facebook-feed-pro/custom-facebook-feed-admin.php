                    ...
                    <tr valign="top">
                        ...
                        <td>
                            <select name="cff_video_action" style="width: 280px;">
                                <option value="post" <?php if($cff_video_action == "post") echo 'selected="selected"' ?> ><?php _e('Embed using HTML5 video player'); ?></option>
                                <!-- Link to the video either on Facebook or whatever the source is: -->
                                <option value="facebook" <?php if($cff_video_action == "facebook") echo 'selected="selected"' ?> ><?php _e('Link to the video'); ?></option>
                            </select>
                        </td>
                    </tr>
...
function cff_admin_style() {
        wp_register_style( 'cff_custom_wp_admin_css', plugin_dir_url( __FILE__ ) . 'css/cff-admin-style.css?11', false, '1.0.0' );
...
function cff_admin_scripts() {
    wp_enqueue_script( 'cff_admin_script', plugin_dir_url( __FILE__ ) . 'js/cff-admin-scripts.js?11' );
...
// delete_option( 'cff_license_data' );
    ...
    if( empty($cff_license) || !isset($cff_license) ) return;
    ...
    if($cff_license_expired || $cff_expire_countdown) {
        ...
        if($cff_expire_countdown){
            ...
            $cff_expired_box_msg = "expires in " . $cff_interval . " days";
        } else {
            $cff_expired_box_classes = "cff-license-expired";
            $cff_expired_box_msg = "has expired";
        }

        _e("
        <div class='".$cff_expired_box_classes."'>
            <p>Hey ".$cff_license_data["customer_name"].", your Custom Facebook Feed Pro license key ".$cff_expired_box_msg.". Click <a href='https://smashballoon.com/checkout/?edd_license_key=".$cff_license."&download_id=".$cff_download_id."' target='_blank'>here</a> to renew your license.</p>
            <a href='javascript:void(0);' id='cff-why-renew-show' onclick='cffShowReasons()'>Why renew?</a>
            <a href='javascript:void(0);' id='cff-why-renew-hide' onclick='cffHideReasons()' style='display: none;'>Hide text</a>
            <div id='cff-why-renew' style='display: none;'>
                ...
                <h4>Extensions</h4>
                <p>In order to utilize the latest plugin extensions you need to be using the latest version of the plugin. Keep your plugin up to date and check out the new extensions as they are released.</p>
            </div>
        </div>
        ...
        function cffShowReasons() {
            ...
            document.getElementById('cff-why-renew-hide').style.display = 'block';
        }
        function cffHideReasons() {
            document.getElementById('cff-why-renew').style.display = 'none';
            document.getElementById('cff-why-renew-show').style.display = 'block';
        ...
        if ( $cff_check_page !== 'cff-top' && $cff_check_page !== 'cff-style' && $cff_check_page !== 'cff-extensions' ) {
            ...
            if($cff_expire_countdown){
                ...
                <div class='".$cff_expired_box_classes."'>
                    ...
                    <a style='float:right; color: #dd3d36; text-decoration: none;' href='?cff_nag_ignore=0'>Dismiss</a>
                </div>
                <script type='text/javascript'>
                function cffShowReasons() {
                    document.getElementById('cff-why-renew').style.display = 'block';
